...
void SendMessage(const unsigned long Interval) {
  ...
  GSMSerial.print('"');
  GSMSerial.print(ParameterList._Phone1);       //发送目标手机号在这里设置
  GSMSerial.print('"');
  GSMSerial.print("\r\n");
  delay(1000);
...
void AT_Commands(String item, String value) {
  ...
  */
  ...
  if (item.equals("AT+PHONE1")) {                          //设置短信接收方手机号码1
    if (!value.equals("")) {
      for (i = 0; i < 12; i++)
        ParameterList._Phone1[i] = value[i];
      saveParameterBean();
      Serial.print("OK");
    } else {
      Serial.print(ParameterList._Phone1);
    }
    Serial.print(_Terminator);
    return;
  }
  if (item.equals("AT+PHONE2")) {                          //设置短信接收方手机号码2
    if (!value.equals("")) {
      for (i = 0; i < 12; i++)
        ParameterList._Phone2[i] = value[i];
      saveParameterBean();
      Serial.print("OK");
    } else {
      Serial.print(ParameterList._Phone2);
    }
    Serial.print(_Terminator);
    return;
  }
  if (item.equals("AT+PHONE3")) {                          //设置短信接收方手机号码3
    if (!value.equals("")) {
      for (i = 0; i < 12; i++)
        ParameterList._Phone3[i] = value[i];
      saveParameterBean();
      Serial.print("OK");
    } else {
      Serial.print(ParameterList._Phone3);
    }
    Serial.print(_Terminator);
    return;
  }
