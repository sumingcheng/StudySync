...
void setup() {
  ...
  GSMSerial.begin(ParameterList._SSBaudRate);
...
void loop() {
  ...
  if (!_BoolAT) {
    ReadVoltageValue(ParameterList._DetectionTime);
...
void SendMessage(const unsigned long Interval) {
  ...
  String PhoneNumber;
  for (i = 0; i < 3; i++) {
    PhoneNumber = ParameterList._Phone[i];
    if (PhoneNumber.length() < 11) {
      ...
      continue;
        ...
        GSMSerial.print('"');
        GSMSerial.print(ParameterList._Phone[i]);       //发送目标手机号在这里设置
        GSMSerial.print('"');
        GSMSerial.print("\r\n");
        delay(1000);
  ...
  */
  ...
  if (item.equals("AT+PHONE")) {                          //设置短信接收方手机号码
    if (!value.equals("")) {
      ...
      P = value.substring(0, index);
      N = value.substring(index + 1);
      for (i = 0; i < 12; i++)
        ParameterList._Phone[P.toInt()][i] =N[i];
      saveParameterBean();
      Serial.print("OK");
    } else {
      Serial.print("PHONE0:");
      Serial.print(ParameterList._Phone[0]);
      Serial.print(_Terminator);
      Serial.print("PHONE1:");
      Serial.print(ParameterList._Phone[1]);
      Serial.print(_Terminator);
      Serial.print("PHONE2:");
      Serial.print(ParameterList._Phone[2]);
    }
    Serial.print(_Terminator);
    return;
  }
